...
def Test(graph, request_EV, CS_origin_list, model_src):
    ...
    agent = PPO(0, update_timestep, gmodel, N_S, N_A, lr_actor, lr_critic, gamma, K_epochs, eps_clip, None)
    random_seed = 0
...
if __name__ == '__main__':
    ...
    # print(sum(hdata)/len(hdata))
    nd = np.array(data)
    print(nd.T)
    result = nd.sum(axis=0)
    print(result.shape)
